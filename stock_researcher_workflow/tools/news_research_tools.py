...
def get_company_news(ticker: str, limit: int = 8) -> str:
    """Fetch recent news headlines, summaries, and URLs for a ticker to support sentiment analysis."""
...
def search_news_context(headline: str) -> str:
    """Search the web for a given news headline to pull full context, source, URL, and related coverage."""
    headers = {"X-API-KEY": os.environ["SERPER_API_KEY"], "Content-Type": "application/json"}
    payload = {"q": headline, "tbm": "nws", "num": 10}
    ...
    lines = [
        f"- {r.get('title')} ({r.get('source')}, {r.get('date')}): {r.get('snippet')}\n  {r.get('link', 'No URL available')}"
        for r in results
    ]
    return "\n".join(lines)